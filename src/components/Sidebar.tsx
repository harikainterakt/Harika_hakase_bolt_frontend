            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-lg text-left transition-all duration-200 ${
                  isActive
                    ? 'bg-gradient-to-r from-blue-50 to-purple-50 text-moduleshade border border-blue-200'
                    : isEnabled
              ...
              >
                <div className="flex items-center">
                  {/* 🔹 All icons always use miconbrandblue */}
                  <Icon className="h-5 w-5 mr-3 text-mskyblueshade" />
                  <span className="font-medium">{item.label}</span>
                </div>

                {/* 🔹 ChevronRight also uses miconbrandblue when active */}
                {isActive && <ChevronRight className="h-4 w-4 text-miconbrandblue" />}
              </button>
            );
          })}
        </div>
